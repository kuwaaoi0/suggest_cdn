...
;(function () {
  ...
  const G = {
    ...
    inputClass: SCRIPT?.dataset?.inputClass || 'my-suggest', // クラスでの自動バインド
    selector:   SCRIPT?.dataset?.suggest || null,            // セレクタでまとめてバインド
  };
  ...
  (function injectCssOnce(){
    ...
    .sg-panel{
      position:absolute;left:0;right:0;top:100%;z-index:2147483000;margin-top:6px;
      background: var(--suggest-bg, #fff);
      color: var(--suggest-fg, inherit);
      font-family: var(--suggest-font-family, inherit);
      font-size: var(--suggest-font-size, 14px);
      border:1px solid var(--suggest-border, #e2e8f0);
      border-radius: var(--suggest-radius, 10px);
      box-shadow: var(--suggest-shadow, 0 10px 30px rgba(0,0,0,.08));
      overflow:hidden
    }
    .sg-item{padding:10px 12px;display:flex;gap:10px;align-items:center;cursor:pointer;line-height:1.4}
    .sg-item:hover,.sg-item.active{background:var(--suggest-active-bg, #f1f5f9)}
    .sg-label{flex:1}
    .sg-badge{font-size:11px;padding:2px 6px;background:#e2e8f0;border-radius:999px;margin-left:6px}
    `;
  ...
  // 入力の見た目 → パネルへ反映
  function applyThemeFromInput(input, panel){
    if (!input || !panel) return;
    const s = getComputedStyle(input);
    panel.style.setProperty('--suggest-font-family', s.fontFamily);
    panel.style.setProperty('--suggest-font-size',   s.fontSize);
    panel.style.setProperty('--suggest-fg',          s.color);
    panel.style.setProperty('--suggest-bg',          s.backgroundColor || '#fff');
    // 枠色は 4辺のうち可視の色を採用
    const borderColor = [s.borderTopColor, s.borderRightColor, s.borderBottomColor, s.borderLeftColor]
      .find(c => c && c !== 'transparent' && c !== 'rgba(0, 0, 0, 0)') || '#e2e8f0';
    panel.style.setProperty('--suggest-border',      borderColor);
    panel.style.setProperty('--suggest-radius',      s.borderTopLeftRadius || '10px');
  }
  ...
  function bindInput(input, baseCfg){
    ...
    // ← 追加: 生成直後＆各タイミングでテーマ同期
    applyThemeFromInput(input, panel);
    input.addEventListener('focus', ()=>applyThemeFromInput(input, panel));
    const themeMo = new MutationObserver(()=>applyThemeFromInput(input, panel));
    themeMo.observe(input, { attributes:true, attributeFilter:['class','style'] });

    function syncPanelWidth(){
      const r = input.getBoundingClientRect();
      panel.style.boxSizing = 'border-box';
      panel.style.width = r.width + 'px';
      panel.style.left = '0px'; // 念のため固定
      panel.style.right = 'auto';
    }
    // 初期・フォーカス・ウィンドウリサイズ・要素サイズ変化で同期
    syncPanelWidth();
    input.addEventListener('focus', syncPanelWidth);
    ...
    function choose(i){
      ...
      try{
        ...
        if (cfg.apiKey) {
          // X-Suggest-Key を付けたいので fetch を優先（ページ遷移中でも送るため keepalive）
          fetch(cfg.click, {
            method: 'POST',
            mode: 'cors',
            credentials: 'omit',
          ...
          fetch(cfg.click, { method:'POST', mode:'cors', credentials:'include', headers: headers(true), body: payload });
        }
      }catch{}
